...
|---|---|---|
| **FY-boundary drop** in `annual_overview.php` (FIXED on this branch) | 192 invoices / $297,823 carry a non-midnight 31-Mar timestamp — **but all are in calendar years 2003–2014**, so **no currently-selectable FY (≈2021+) is affected today** | `Date BETWEEN '…-04-01' AND '…-03-31'` treats `'…-03-31'` as midnight, so a timestamped 31-Mar invoice falls out of both the FY it belongs to and the next. Latent rather than active: `Invoices.Date` defaults to `current_timestamp()`, so any invoice **created on 31 March** (a real scenario at FY close) would be dropped from that FY. Fixed via a half-open range `[from, firstDayOfNextFY)`. Severity: Medium (correctness; bites only at FY close). |
| **Zero-date invoice** | ≥1 invoice with `Date = '0000-00-00 00:00:00'` | Invalid date; `strtotime()`/`DATE_FORMAT` on it misbehave. Worth correcting the row. Minor. |
