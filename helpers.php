...
function default_email_templates(): array {
    return [
        ...
        'transmittal' => [
            'label'        => 'Drawing transmittal (coordination — sent to stakeholders on a revision)',
            'placeholders' => 'stakeholder_name, role_label, project_name, job_name, revision, commit_message, sender_name, nzbc_tags, file_list, review_url',
            'subject'      => 'Drawing revision for review — {job_name} (Rev {revision})',
            'text'         => "Dear {stakeholder_name},\r\n\r\nA revision has been issued on {job_name} ({project_name}) that may affect your scope ({role_label}).\r\n\r\nRevision: {revision}\r\nWhat changed: {commit_message}\r\nBuilding-code areas flagged: {nzbc_tags}\r\n\r\nDocuments in this transmittal:\r\n{file_list}\r\n\r\nPlease REVIEW and ACKNOWLEDGE via this private link (no login required):\r\n{review_url}\r\n\r\nAcknowledging confirms you've seen this revision and assessed its impact on your design. If it affects your work, reply via the link's comment box or to this email so we can coordinate before it goes to council.\r\n\r\nThank you,\r\n{sender_name}\r\nCADViz Limited",
            'html'         => '<p>Dear {stakeholder_name},</p><p>A revision has been issued on <strong>{job_name}</strong> ({project_name}) that may affect your scope (<em>{role_label}</em>).</p><table style="border-collapse:collapse;font-size:14px;margin:10px 0"><tr><td style="padding:3px 10px 3px 0;color:#666">Revision</td><td style="padding:3px 0"><strong>{revision}</strong></td></tr><tr><td style="padding:3px 10px 3px 0;color:#666">What changed</td><td style="padding:3px 0">{commit_message}</td></tr><tr><td style="padding:3px 10px 3px 0;color:#666">Code areas flagged</td><td style="padding:3px 0">{nzbc_tags}</td></tr></table><p style="color:#666;margin-bottom:4px">Documents in this transmittal:</p>{file_list}<p style="margin:18px 0"><a href="{review_url}" style="background:#9B9B1B;color:#fff;padding:10px 18px;border-radius:3px;text-decoration:none;font-weight:bold">Review &amp; acknowledge this revision &rarr;</a></p><p style="font-size:12px;color:#666">Acknowledging confirms you&rsquo;ve seen this revision and assessed its impact on your design. If it affects your work, use the comment box on that page (or reply to this email) so we can coordinate before it goes to council. No login required &mdash; the link is private to you.</p><p>Thank you,<br><strong>{sender_name}</strong><br>CADViz Limited</p>',
        ],
        'statement_manual' => [
        ...
        'partial_payment_thanks' => [
            'label'        => 'Partial payment — thanks, balance still owed',
            'placeholders' => 'name, client_name, invoice_no, amount_paid, amount_remaining, due_date, days_overdue, online_url',
            'subject'      => 'Thanks for the partial payment on Invoice {invoice_no} — balance still due',
            'text'         => "Dear {name},\r\n\r\nThank you for the recent payment of {amount_paid} towards invoice {invoice_no}. Much appreciated.\r\n\r\nThis invoice still has a remaining balance of {amount_remaining} which is now {days_overdue} days past the due date ({due_date}). Please arrange the balance at your earliest convenience.\r\n\r\nView / pay online: {online_url}\r\n\r\nIf there's anything to discuss about the balance please reply to this email \xE2\x80\x94 we'd rather have a quick chat than send another reminder.\r\n\r\n{bank_details}\r\n\r\n{kind_regards}\r\n",
            'html'         => '<p>Dear {name},</p><p>Thank you for the recent payment of <strong>{amount_paid}</strong> towards invoice <strong>{invoice_no}</strong>. Much appreciated.</p><p>This invoice still has a remaining balance of <strong>{amount_remaining}</strong> which is now <strong>{days_overdue} days past the due date</strong> ({due_date}). Please arrange the balance at your earliest convenience.</p><p><a href="{online_url}">View / pay invoice online</a></p><p>If there&rsquo;s anything to discuss about the balance please reply to this email &mdash; we&rsquo;d rather have a quick chat than send another reminder.</p>{bank_details_html}{kind_regards_html}',
        ],
    ];
}
