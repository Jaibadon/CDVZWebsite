...
- **`project_stages.php`** — admin "stages/tasks/quote builder" page. Loads `$quoteStatus` (NULL/draft = free editing, 'accepted' = locked) and includes `stages_editor.php`. Has Accept Quote / Reset to Draft / **📋 Duplicate this project** buttons + Print Original Quote / Quote (+variations) / Variations / Checklist links. **PaymentOption picker** with values 1=20th of next month, 2=invoice date + 7 days, 3=invoice date itself. **Duplicate** copies the Projects row + original-quote stages + non-removed original-quote tasks (preserving `Quoted_Rate` snapshots) into a fresh project with `Quote_Status = NULL` (draft) and a "Copy of …" JobName prefix; variations, invoices, and timesheets are NOT copied (they belong to the original's lifecycle).
...
- **`spec_admin.php`** — combined CRUD for `Spec_Cats` (top-level: Wall, Roof, Joinery, Project Process, …) and `Spec_SubCats` (children: Wall Cladding, Roof Framing, Window Schedule, Project Setup & Admin, …). **Replaces** the old `SUBCAT_TYPES.php` + `subcat_type_*.php` files (all deleted). Each `Tasks_Types.Spec_Subcat_ID` links a task to a sub-cat — that's how the Generate-from-spec wizard groups them.
- **`quote_from_spec.php`** — wizard reachable from `project_stages.php` (📐 Generate from spec button, draft mode only). Step 1 = pick categories the project has (multi-checkbox over `Spec_Cats`). Step 2 = page reloads showing every task type under those cats' sub-cats, all checked by default; admin un-ticks anything they don't want. Submit = creates `Project_Stages` rows on demand (one per distinct `Stage_ID` among the chosen tasks) and `Project_Tasks` rows pointing at them. Quoted_Rate snapshotted at TBA. Append-only — existing tasks stay; refuses to run on accepted projects.
...
- `bankfeed_match.php` — matcher with **liberal invoice-ref regex** plus a **non-client description blacklist**:
  - **Prefixed**: `CAD-01234`, `caD01234`, `INV-1234`, `inv 12345`. Multi-invoice in one description (`cad12345 cad6789`) walks all matches.
  - **Bare**: plain `12345` / `1234`. Validated against `Invoices.Invoice_No`. Skipped entirely when the description text contains a non-client keyword — current list covers NZ tax/govt (IRD, ACC, MSD, GST, PAYE, studylink, companies office, withholding, levy), bank-internal credits (interest, fee, service charge, transfer, reversal, correction, adjustment, refund, rebate, chargeback, overdraft, atm, eftpos, direct debit/credit), payroll (salary, wages, payroll, kiwisaver, super, holiday/leave pay, bonus, commission, dividend, termination, redundancy), property (rent, rates, council, body corporate), money-movement (loan, mortgage, repayment, fx, foreign currency). All keywords are case-insensitive and dot-tolerant for "I.R.D."-style abbreviations. Add new keywords by editing the regex in `extract_invoice_refs()` — no other code changes needed.
  - Both prefixed and bare matches allocate identically (with full partial-payment support); they're tagged `auto-ref-prefixed` vs `auto-ref-bare` in `Bank_Allocations.allocated_by` so you can audit bare-match outcomes after the fact.
  - **Does NOT touch `Invoices.Paid`** directly — only writes `Invoices.AmountPaid` cache + `Bank_Allocations` rows. Auto-flips `Paid=1` only when AmountPaid ≥ gross AND DatePaid is null (separate guard, see below).
- `bankfeed_reconcile.php` — manual queue UI for transactions the matcher couldn't fully resolve. Two re-match buttons at the top:
  - **↻ Re-run auto-match (new only)**: walks unmatched / partially-matched transactions only. Cheap, idempotent, picks up newly-imported rows.
  - **🔄 Re-evaluate ALL auto-matches**: wipes every `Bank_Allocations` row with `auto=1`, resets the affected transactions to `unmatched`, and re-runs the matcher. Use this after tweaking the regex or blacklist in `extract_invoice_refs()` to clean up false-positive matches that the previous logic created. Manual allocations (`auto=0`) are preserved untouched.
